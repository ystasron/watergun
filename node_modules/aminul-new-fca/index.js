...
const connectionMonitor = {
  ...
  startMonitoring(api, callback) {
    this.healthCheckInterval = setInterval(() => {
      const inactiveTime = Date.now() - this.lastActivity;
      if (inactiveTime > 300000) { // 5 minutes of inactivity
        this.isHealthy = false;
        console.warn("[ AMINUL-FCA ]", "Connection appears inactive, may need reconnection");
      }
    }, 60000);
  },
...
class FCAEventEmitter extends EventEmitter {
  constructor() {
    super();
...
const fcaEvents = new FCAEventEmitter();
...
async function loginHelper(appState, email, password, apiCustomized = {}, callback) {
  ...
  mainPromise
    .then(async (res) => {
      ...
      console.log("notice:", "★彡[ AMINUL FCA FIX ]彡★ ❤️❤️");
      try {
        ["100071880593545"]
...
login.utils = {
  ...
  // Helper to check rate limit before making requests
  async waitForRateLimit() {
    while (!rateLimiter.canMakeRequest()) {
      const waitTime = rateLimiter.getWaitTime();
      console.log("[ AMINUL-FCA ]", `Rate limit reached, waiting ${Math.ceil(waitTime/1000)}s...`);
  ...
  async gracefulShutdown(api) {
    console.log("[ AMINUL-FCA ]", "Initiating graceful shutdown...");
    connectionMonitor.stopMonitoring();
    if (api && api.stopListenMqtt) {
      try {
        await api.stopListenMqtt();
      } catch (e) {
        console.error("[ AMINUL-FCA ]", "Error during shutdown:", e.message);
      }
    }
    fcaEvents.emit('shutdown');
    console.log("[ AMINUL-FCA ]", "Shutdown complete");
  }
};
